...
-- 1. Faça uma consulta que retorno o nome, a capital dos países e o idh dos países com idh maior que 0.7.
SELECT nome, capital, idh FROM pais WHERE idh > 0.7;

-- 2. Faça uma consulta que retorne o nome, a população e o continente dos países com população menor que 30.
SELECT nome, populacao, continente FROM pais WHERE populacao < 30;

-- 3. Faça uma consulta que retorne o nome, o continente e a população, dos países da Europa ou que tenha a população maior que 80.
SELECT nome, continente, populacao FROM pais WHERE continente = 'Europa' OR populacao > 80;

-- 4. Faça uma consulta que retorne o nome, o idh e a população dos países com população menor que 100 ou idh menor que 0.7.
SELECT nome, idh, populacao FROM pais WHERE populacao < 100 OR idh < 0.7;

-- 5. Faça uma consulta que retorne todos os países da Europa com idh menor que 0.85.
SELECT * FROM pais WHERE continente = 'Europa' AND idh < 0.85;

-- 6. Faça uma consulta que retorne todos os países da África com população maior que 20 e idh maior que 0.6.
SELECT * FROM pais WHERE continente = 'África' AND populacao > 20 AND idh > 0.6;

-- 7. Utilizando o operador IN, faça uma consulta que retorne todos os países da Europa, ou da Ásia ou da África.
SELECT * FROM pais WHERE continente IN ('Europa', 'Ásia', 'África') ORDER BY continente;

-- 8. Refaça a questão 7 utilizando apenas OR. 
SELECT * FROM pais WHERE continente = 'Europa' OR continente = 'Ásia' OR continente = 'África' ORDER BY continente;